<div class="footer-content clearfix">
	<ul class="social clearfix left">
		<li>
			<a class="fab" href="https://www.facebook.com/RidnaZhitnitsaTM" target="_blank"></a>
		</li>
		<li>
			<a class="vk" href="https://vk.com/ridnazhitnitsa" target="_blank"></a>
		</li>
		<li>
			<a class="od" href="#"></a>
		</li>
		<li>
			<a class="ram" href="#"></a>
		</li>
		<li>
			<a class="fl" href="#"></a>
		</li>
		<li>
			<a class="ya" href="#"></a>
		</li>
		<li>
			<a class="g" href="https://plus.google.com/115881097286613802999/posts" target="_blank"></a>
		</li>
		<li>
			<a class="tv" href="#"></a>
		</li>
	</ul>
	<a data-reveal-id="contents_popup" href="#" content_id="42" class="show-popup"><div class="map right"></div></a>
                
	<div class="contacts left">
		<h3><?= __("contacts.title"); ?></h3>
		<span><?= __("contacts.phone_information.title"); ?></span>
		<span><?= __("contacts.phone_information.use"); ?></span> 
		<span><?= __("contacts.phone_information.quality"); ?></span>
		<span><?= __("contacts.phone_information.complains"); ?></span>
		<span> 
			<a target="_blank" href="mailto:ridnazhitnitsa@gmail.com">
				E-mail: ridnazhitnitsa@gmail.com
			</a>
		</span>
	</div>
	<div class="contacts-dop left">
		 <span><?= __("contacts.phone_shopping.title"); ?></span>
		 <span><?= __("contacts.phone_shopping.mob1"); ?></span>
		 <span><?= __("contacts.phone_shopping.mob2"); ?></span>
		 <span><?= __("contacts.phone_shopping.mob3"); ?></span>

		 <span><a href="http://umimarket.com.ua/index.php?route=product/category&amp;path=119" style="color: #332277;" target="_blank"><?= __("contacts.shop_for"); ?></a></span>
	</div>
        <div class='yand-metr left' >
            <!-- Yandex.Metrika informer -->
            <a href="http://metrika.yandex.ru/stat/?id=22608880&amp;from=informer"
               target="_blank" rel="nofollow"><img src="//bs.yandex.ru/informer/22608880/2_1_EDEAEAFF_CDCACAFF_0_pageviews"
                                                style="width:80px; height:31px; border:0;" alt="Яндекс.Метрика" title="Яндекс.Метрика: данные за сегодня (просмотры)" onclick="try{Ya.Metrika.informer({i:this,id:22608880,lang:'ru'});return false}catch(e){}"/></a>
            <!-- /Yandex.Metrika informer -->
        </div>        
        <div class='mail-metr left' >
            <!-- Rating@Mail.ru logo -->
            <a href="http://top.mail.ru/jump?from=2413421">
                <img src="//top-fwz1.mail.ru/counter?id=2413421;t=479;l=1" 
                     style="border:0;" height="31" width="88" alt="Рейтинг@Mail.ru" /></a>
            <!-- //Rating@Mail.ru logo -->
        </div>        
</div>