...
<? $content = Model_Content::find($content->id); 
	$localcontent = $content->get_translation($local_id); ?>
<?php echo Form::open(array('enctype'=>'multipart/form-data', 'class' => 'content-form')); ?>
	<fieldset>
		...
		<div class="clearfix">
			...
			<div class="input">
				<?= Form::input('name', Input::post('name', isset($localcontent) ? $localcontent->name : ''), array('class' => 'span12')); ?>
			</div>
		</div>
		<div class="clearfix">
			<?= Form::label('Краткое описание', 'short_description'); ?>
			<div class="input">
				<?= Form::textarea('short_description', Input::post('short_description', isset($localcontent) ? $localcontent->short_description : ''), array('class' => 'span12', 'rows' => 12)); ?>
			</div>
		</div>
		<div class="clearfix">
			<?= Form::label('Полное описание', 'description'); ?>
			<div class="input">
				<?= Form::textarea('description', Input::post('description', isset($localcontent) ? $localcontent->description : ''), array('class' => 'span12', 'rows' => 12)); ?>
			</div>
		</div>
		<div class="clearfix">
